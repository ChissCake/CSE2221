...
public final class PizzaOrderManager {
    ...
    private static void getPriceMap(String fileName,
        ...
        while (!fileIn.atEOS()) {
            ...
            for (int i = 0; i < input.length(); i++) {
                if (input.charAt(i) == ',') {
                    item = input.substring(0, i);
                    price = Integer.parseInt(input.substring(i + 1));
                    priceMap.add(item, price);
                }
            }
        }
        fileIn.close();
    ...
    private static int getOneOrder(SimpleReader input,
        ...
        while (inputLine.length() > 0) {
            ...
            if (sizePriceMap.hasKey(inputLine)) {
                price += sizePriceMap.value(inputLine);
            }

            if (toppingPriceMap.hasKey(inputLine)) {
                price += toppingPriceMap.value(inputLine);
            }
    ...
    private static void putPrice(SimpleWriter output, int price) {
        ...
        output.println("$" + price / 100 + "." + price % 100);
