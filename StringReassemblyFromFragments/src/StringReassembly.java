...
public final class StringReassembly {
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet,
        ...
        Set<String> tempSet = new Set1L<String>();
        boolean subStrCheck = false;
        while (strSet.size() > 0) {
            String tempStr = strSet.removeAny();

            /*
             * check if any strings are substrings of str if so, then dont add
             * If the strings are equal, then add the original string and dont
             * flag for a substring
             */
            if (str.indexOf(tempStr) == -1 || tempStr.compareTo(str) == 0) {
                tempSet.add(tempStr);
            }
            /*
             * If str exists as a substring in tempStr or is a super string,
             * then dont add the tempStr and flag that a substring/superstring
             * exists
             */
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        Set<String> readInChecked = new Set1L<String>();
        /*
         * continue while the then of stream has not been reached
         */
        while (!input.atEOS()) {
            addToSetAvoidingSubstrings(readInChecked, input.nextLine());
        }

        return readInChecked;
    }
    ...
    public static void printWithLineSeparators(String text, SimpleWriter out) {
        ...
        /*
         * use replaceAll to replace all '~' with new line characters in
         * tempString
         */
